...
`AMD_WS()` exits with `NOTSURE` / `INTERR` and a clear log line; the module's
...
| Connect loop never exited on `CLIENT_CONNECTION_ERROR`. | An unreachable AMD server parked answered calls in silence. | Connect failure returns immediately with `NETERR`; repeated failures are logged once per 10 s per host. |
...
| Result matched with `strstr()`: `"AMD"` matched the brand string `AMDY`, `NOT_HUMAN` matched `HUMAN`; replies >= 256 bytes or fragmented were ignored. | Live humans hung up on (`MACHINE`), machines routed to agents, long results silently lost. | Token-based classification (see [protocol.md](protocol.md)); fragments reassembled; JSON `status`/`result`/`classification` honoured. |
...
| Fixed 500 ms chunks only; the documented 500/1000/1500/2000/3000/4000 ms schedule was not implemented; no `{"eof":1}`. | Diverged from `amd.py`. | Schedule implemented as configured (`send_schedule`, `chunk_bytes`); `{"eof":1}` sent. |
...
| `AMDCAUSE` values (`CONNECT_FAILED`, `CONNECTION_TIMEOUT`, `CONTEXT_FAILED`, `TIMEOUT`, raw server text) did not match the `NETERR`/`INTERR` the production 8370 dialplan tests for. | The fallback to stock `AMD()` never triggered during an outage. | Frozen vocabulary: `NETERR`, `INTERR`, `AUDIO_TIMEOUT`, `NO_AUDIO_TIMEOUT`, `HANGUP`, or the classification token. Raw text moved to `AMDRESPONSE`. |
