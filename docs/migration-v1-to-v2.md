...
| Result other than `HUMAN`/`MACHINE` (e.g. the server says `HONEYPOT`) | `AMDSTATUS=NOTSURE`, `AMDCAUSE=<raw text>` | `AMDSTATUS=AMDCAUSE=<token>` (`HONEYPOT`). Check `VD_amd.agi` container entries and any dialplan branch that expected `NOTSURE` for these calls |
