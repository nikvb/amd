...
#
# Generated by tools/gen-installer.sh from commit b13ce07-dirty on 2026-09-24 (installer version b13ce07). DO NOT EDIT: run 'make installer'.
#
...
AMD_WS_INSTALLER_VERSION='b13ce07'
AMD_WS_SOURCE_COMMIT='b13ce07-dirty'
MODULE=app_amd_ws
APP=AMD_WS
...
; Per-call event timeline in the Asterisk log at verbose 3: connect, first audio
; frame, every chunk sent, every server reply, result - each with the offset in
; ms from the start of AMD_WS().  Off by default; the dialplan option v turns it
; on for one call.  Without tracing the same lines are available with
; 'core set debug 2 app_amd_ws'.
trace=no
...
; Extra keys for the server, spliced verbatim into the "config" JSON frame (must
; be a JSON object).  Empty = the exact amd.py frame.  amd_server understands,
; for example:
;   {"short_no_greeting":true}       finalise at the 32k AMD stage instead of
;                                    waiting for 36 000 stripped samples (4.5 s
;                                    of speech) and the greeting model: ~0.5 s
;                                    faster, but the verdict no longer says
;                                    WHICH machine (plain AMD instead of
;                                    NUMBERSAMD/OTHERAMD/...).
;   {"detection_mode":"aggressive"}  threshold profile: aggressive | balanced
;                                    (default) | conservative - latency vs leak.
;   {"max_detection_time":8.0}       server wall-clock deadline in seconds.
;   {"stage_results":true}           per-stage progress frames (STAGE-...); the
;                                    module treats them as acks.
; Several keys: {"short_no_greeting":true,"detection_mode":"aggressive"}
extra_config=
__APP_AMD_WS_EMBED_EOF__
