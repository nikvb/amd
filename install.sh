...
#
# Generated by tools/gen-installer.sh from commit 751ad5b on 2026-09-22 (installer version 751ad5b). DO NOT EDIT: run 'make installer'.
#
...
AMD_WS_INSTALLER_VERSION='751ad5b'
AMD_WS_SOURCE_COMMIT='751ad5b'
MODULE=app_amd_ws
APP=AMD_WS
...
; Hard bound for the whole lookup (connect + query, and waiting for the
; shared connection).  Connect/read/write timeouts are set from this value
; rounded up to whole seconds (minimum 1).  After a failure the DB is
; skipped for 5 s so a dead DB costs one timeout, not one per call; failures
; are logged at WARNING at most once per minute and the call continues
; without phone.
db_timeout_ms=1000
...
; ';' comments and '=>' inside values, like ViciDial itself.
